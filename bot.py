...
def start(message):
    text = ("👋 Привет! Я бот клана Black Rose.\n\n"
            "🎮 Нажми кнопку «Добавить свой ник» и напиши свой игровой ник.")
    bot.send_message(message.chat.id, text, reply_markup=nick_button())
...
def save_nick_command(message):
    try:
        ...
        bot.reply_to(message, f"✅ Игровой ник {game_nick} сохранён!")
...
def save_nick(message):
    ...
    save_nicks(game_nicks)
    bot.send_message(message.chat.id, f"✅ Ник {nick} сохранён!", reply_markup=types.ReplyKeyboardRemove())
...
def delete_nick(message):
    ...
    try:
        ...
        bot.reply_to(message, f"✅ Игрок {game_nick} удалён из базы.")
        list_nicks(message)
...
def start_voting():
    ...
    bot.send_message(
        CHAT_ID,
        f"📢 Голосование на КВ началось!\n\n"
        f"Каждому игроку пришло сообщение в личку.\n"
        f"Итоги будут завтра в 07:00."
    )
    ...
    for user_id, data in game_nicks.items():
        try:
            ...
            bot.send_message(
                user_id,
                f"⚔️ КВ завтра. Ты идёшь?\n\n"
                f"Твой ник: {data['game_nick']}",
                reply_markup=markup
            )
            sent += 1
        except:
